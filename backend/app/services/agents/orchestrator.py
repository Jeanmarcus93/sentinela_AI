...
class AgentOrchestrator:
    ...
    def get_system_health(self) -> Dict[str, Any]:
        ...
        load_info = self.get_agent_load_balancing_info()
        
        # Calcular saúde geral
        system_healthy = True
        issues = []
        
        # Verificar se todos os agentes estão funcionais
        if stats["registered_agents"] == 0:
            system_healthy = False
            issues.append("Nenhum agente registrado")
        
        # Verificar taxa de erro
        if stats["overall_success_rate"] < 0.5:  # Menos de 50% de sucesso
            system_healthy = False
            issues.append(f"Taxa de sucesso baixa: {stats['overall_success_rate']:.2%}")
        
        # Verificar sobrecarga
        avg_load = sum(info["current_load"] for info in load_info.values()) / max(len(load_info), 1)
        if avg_load > 0.9:  # Mais de 90% de carga
            system_healthy = False
            issues.append(f"Sistema sobrecarregado: {avg_load:.1%}")
        
        return {
            "healthy": system_healthy,
            "issues": issues,
            "stats": stats,
            "load_balancing": load_info,
            "timestamp": time.time()
        }
